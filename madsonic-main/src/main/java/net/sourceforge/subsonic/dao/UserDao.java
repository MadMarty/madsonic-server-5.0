...
public class UserDao extends AbstractDao {
    ...
    private static final String USER_COLUMNS = "username, password, email, ldap_authenticated, bytes_streamed, bytes_downloaded, bytes_uploaded, group_id, locked";
    private static final String USER_SETTINGS_COLUMNS = "username, locale, theme_id, final_version_notification, beta_version_notification, " +
            "main_caption_cutoff, main_track_number, main_artist, main_album, main_genre, main_mood, " +
            "main_year, main_bit_rate, main_duration, main_format, main_file_size, " +
            "playlist_caption_cutoff, playlist_track_number, playlist_artist, playlist_album, playlist_genre, playlist_mood, " +
    ...
    public void createUser(User user) {
        ...
        update(sql, user.getUsername(), encrypt(user.getPassword()), user.getEmail(), user.isLdapAuthenticated(),
                user.getBytesStreamed(), user.getBytesDownloaded(), user.getBytesUploaded(), user.getGroupId(), user.isLocked());
        writeRoles(user);
    }
    ...
    public void updateUser(User user) {
        String sql = "update user set password=?, email=?, ldap_authenticated=?, bytes_streamed=?, bytes_downloaded=?, bytes_uploaded=?, group_id=?, locked=? " +
                "where username=?";
        getJdbcTemplate().update(sql, new Object[]{encrypt(user.getPassword()), user.getEmail(), user.isLdapAuthenticated(),
                ...
                user.getGroupId(), user.isLocked(), user.getUsername() });
        writeRoles(user);
    }
    ...
    private class UserRowMapper implements ParameterizedRowMapper<User> {
        public User mapRow(ResultSet rs, int rowNum) throws SQLException {
            User user = new User(rs.getString(1), decrypt(rs.getString(2)), rs.getString(3), rs.getBoolean(4),
                                 rs.getLong(5), rs.getLong(6), rs.getLong(7), rs.getInt(8), rs.getBoolean(9) );
            readRoles(user);
            return user;
        }
    }
