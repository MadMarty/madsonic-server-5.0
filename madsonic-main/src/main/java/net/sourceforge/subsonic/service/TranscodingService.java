...
public class TranscodingService {
    ...
    public Parameters getParameters(MediaFile mediaFile, Player player, Integer maxBitRate, String preferredTargetFormat,
                                    VideoTranscodingSettings videoTranscodingSettings, boolean prepare) {
        ...
        if ( player.getName() != null ) {
            if ( hashClientsFormats.containsKey(player.getName().toUpperCase()) ) { 
                if ( hashClientsFormats.get(player.getName().toUpperCase()).contains(mediaFile.getFormat().toUpperCase()) ) {
                    if ( bitRate == null || maxBitRate == null || bitRate < maxBitRate ) {
                    	if (!prepare){
                        LOG.debug("## " + player.getName() + "-Client supports natively " + mediaFile.getFormat() + ",  Download file." );
                    	}
