...
class Madsonic:
    bitrates = [200, 300, 400, 500, 700, 1000, 1200, 1500, 2000, 3000, 5000]
    def __init__(self, server, user, password):
        self.server = server
        self.user = user
        self.password = password
        self.api_version = '1.10.5'
    ...
    def get_starred(self):
        payload = self.__get_json('getStarred.view')
        if payload:
            songs = self.listify(payload['starred']['song'])
            self.display_music_directory(songs)
            Addon.end_of_directory()

    def get_lastplayed(self):
        payload = self.__get_json('getLastplayedSongs.view')
        if payload:
            songs = self.listify(payload['lastplayed']['song'])
            self.display_music_directory(songs)
            Addon.end_of_directory()

    def get_newadded(self):
        payload = self.__get_json('getNewaddedSongs.view')
        if payload:
            songs = self.listify(payload['newadded']['song'])
            self.display_music_directory(songs)
            Addon.end_of_directory()
