...
def show_dialog(details, title='Madsonic', is_error=False):
    error = ['', '', '']
    text = ''
...
def add_music_item(item_id, infolabels, img='', fanart='', total_items=0):
    ...
    url = build_plugin_url({'mode': 'play', 'id': item_id})
    log('adding item: %s - %s' % (infolabels['title'], url))
    listitem = xbmcgui.ListItem(infolabels['title'], iconImage=img, thumbnailImage=img)
    listitem.setInfo('music', infolabels)
    listitem.setProperty('IsPlayable', 'true')
    listitem.setProperty('fanart_image', fanart)
    xbmcplugin.addDirectoryItem(plugin_handle, url, listitem, isFolder=False, totalItems=total_items)
...
def add_directory(url_queries, title, img='', fanart='', total_items=0):
    ...
    listitem.setProperty('fanart_image', fanart)
    xbmcplugin.addDirectoryItem(plugin_handle, url, listitem, isFolder=True, totalItems=total_items)
...
addon = xbmcaddon.Addon(id='plugin.video.madsonic')
